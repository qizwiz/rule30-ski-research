...
-- Tail adapter from certificate checks in explicit non-beyond-boundary form
-- (`¬(2n+2 < i)`) to pointwise-difference witnesses.
theorem rule30CenterRec_tail_pointwise_diff_witness_of_tail_cert_not_two_mul_add_two_lt
    (A : Algorithm Rule30State)
    (hTailCertNotTwoMulAddTwoLt :
      forall n i,
        3 < n ->
        ¬ (2 * n + 2 < i) ->
        ¬ (A.observes (n + 1) i) ->
  ...
  have hLe' : i <= 2 * n + 2 := by
    simpa [Nat.mul_add, Nat.add_assoc, Nat.add_comm, Nat.add_left_comm] using hLe
  rcases hTailCertNotTwoMulAddTwoLt n i hGt (Nat.not_lt_of_ge hLe') hNotObs with
      ⟨c, hCn, hCi, hValid, hCheck⟩
  have hEss : essentialByWitness (rule30CenterRec (n + 1)) i := by
    rw [← hCn, ← hCi]
    exact essentialByWitness_of_rule30WitnessCertValid_true c hValid hCheck
  exact pointwise_diff_witness_of_essentialByWitness
...
-- Full next-generation pointwise-difference witness family from:
-- bounded concrete construction (`n <= 3`) + certificate-checked tails in
-- explicit non-beyond-boundary form (`¬(2n+2 < i)`).
theorem rule30CenterRec_next_gen_pointwise_diff_witness_of_le_three_or_tail_cert_not_two_mul_add_two_lt
    (A : Algorithm Rule30State)
    (hTailCertNotTwoMulAddTwoLt :
      forall n i,
        3 < n ->
...
-- Recursive-target no-skip closure adapter for certificate-checked tail
-- hypotheses in explicit non-beyond-boundary form (`¬(2n+2 < i)`).
theorem observes_required_of_rule30CenterRec_base_and_tail_cert_not_two_mul_add_two_lt
    (A : Algorithm Rule30State)
    (h_obs_det :
      forall n s1 s2, agreesOnObserved rule30Cell A n s1 s2 -> A.run n s1 = A.run n s2)
    (h_exact_rec : exactFor A rule30CenterRec)
