...
| O2 | Rule 30 indistinguishability witness discharging `h_witness` | Open | `prize3/lean/Prize3Model.lean` (interface + adapters) | Small-horizon witness seeds and induction scaffolds now cover base construction through `n <= 4`, but the concrete next-generation witness-construction proofs remain open. |
...
1. Add concrete next-generation pointwise-difference witness-construction lemmas that can instantiate `hConcreteNextRec` for recursive target propagation.
2. Compose those lemmas through `rule30CenterRec_witness_all_of_small_horizon_and_pointwise_next` to reduce O2 from scaffold-complete to witness-complete.
3. Use the resulting witness-complete path to tighten O3/O4 composition notes without upgrading theorem status.
