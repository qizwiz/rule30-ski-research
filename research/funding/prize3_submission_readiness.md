...
- Proved (in-repo):
  ...
  - Tail-aware adapter composition is formalized: bounded witnesses plus a tail hypothesis (`3 < n`) assemble full next-generation witness/no-skip interfaces in both boundary and non-beyond-boundary forms, with explicit shape-conversion from `¬(2n+2 < i)` tails into boundary tails, from tail essentiality hypotheses into pointwise-difference tails, and from tail certificate-check hypotheses into pointwise-difference tails in both boundary and non-beyond-boundary certificate shapes (`flipAtState_agree_except`, `pointwise_diff_witness_of_essentialByWitness`, `rule30CenterRec_next_gen_pointwise_diff_witness_of_le_three_or_gt_three`, `rule30CenterRec_next_gen_pointwise_diff_witness_not_two_mul_add_two_lt_of_le_three_or_gt_three`, `rule30CenterRec_tail_pointwise_diff_witness_of_tail_not_two_mul_add_two_lt`, `rule30CenterRec_tail_pointwise_diff_witness_of_tail_essentialByWitness`, `rule30CenterRec_tail_pointwise_diff_witness_of_tail_cert`, `rule30CenterRec_tail_pointwise_diff_witness_of_tail_cert_not_two_mul_add_two_lt`, `rule30CenterRec_next_gen_pointwise_diff_witness_of_le_three_or_tail_cert`, `rule30CenterRec_next_gen_pointwise_diff_witness_of_le_three_or_tail_cert_not_two_mul_add_two_lt`, `observes_required_of_rule30CenterRec_base_and_tail_pointwise_next`, `observes_required_of_rule30CenterRec_base_and_tail_not_two_mul_add_two_lt`, `observes_required_of_rule30CenterRec_base_and_tail_essentialByWitness`, `observes_required_of_rule30CenterRec_base_and_tail_cert`, `observes_required_of_rule30CenterRec_base_and_tail_cert_not_two_mul_add_two_lt`, `observes_required_next_gen_rule30CenterRec_of_pointwise_diff_le_three_or_gt_three`, `observes_next_gen_of_not_two_mul_add_two_lt_rule30CenterRec_of_pointwise_diff_le_three_or_gt_three`).
