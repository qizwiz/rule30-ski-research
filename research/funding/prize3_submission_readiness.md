...
- Proved (in-repo):
  ...
  - Tail-aware adapter composition is formalized: bounded witnesses plus a tail hypothesis (`3 < n`) assemble a full next-generation witness interface and downstream no-skip bridge (`rule30CenterRec_next_gen_pointwise_diff_witness_of_le_three_or_gt_three`, `observes_required_of_rule30CenterRec_base_and_tail_pointwise_next`).
