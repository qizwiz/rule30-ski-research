...
- Proved (in-repo):
  ...
  - Tail-aware adapter composition is formalized: bounded witnesses plus a tail hypothesis (`3 < n`) assemble full next-generation witness/no-skip interfaces in both boundary and non-beyond-boundary forms (`rule30CenterRec_next_gen_pointwise_diff_witness_of_le_three_or_gt_three`, `rule30CenterRec_next_gen_pointwise_diff_witness_not_two_mul_add_two_lt_of_le_three_or_gt_three`, `observes_required_of_rule30CenterRec_base_and_tail_pointwise_next`, `observes_required_next_gen_rule30CenterRec_of_pointwise_diff_le_three_or_gt_three`, `observes_next_gen_of_not_two_mul_add_two_lt_rule30CenterRec_of_pointwise_diff_le_three_or_gt_three`).
