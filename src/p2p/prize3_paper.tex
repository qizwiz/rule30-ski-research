...
\begin{abstract}
\textbf{What this paper unconditionally proves.}  We prove two boundary theorems: the
leftmost and rightmost positions of the width-$n$ light cone of Rule~30 are each
\emph{essential} for all $n$ --- meaning there exists an initial configuration whose
$n$-step center-cell output changes when that boundary cell is flipped.  From these,
a clean Lean~4 induction establishes that \emph{every} position $k \in \{0,\ldots,2n\}$
is essential, conditional on one open lemma (\texttt{subcaseB\_resolution\_ge3087},
described below).  The resulting block-sensitivity bound
$\mathrm{bs}(\mathtt{rule30}_n) \geq n$ yields an $\Omega(n)$ \emph{query-complexity}
lower bound for the \emph{general-input} problem: any algorithm computing
$\mathtt{rule30}_n(c)$ for arbitrary $c \in \{0,1\}^{2n+1}$ must read all $2n+1$
input bits in the worst case.

\textbf{The one open lemma.}  The induction reduces to showing that, for all $n \geq 3087$
and every ``active'' even interior position $m$, if the spike-only configuration gives
center output~0 while the two-spike configuration gives~1, then a valid sensitivity
witness exists at $m$.  This is \texttt{subcaseB\_resolution\_ge3087}.  It is formally
verified in Lean~4 for $n \leq 3086$ via \texttt{native\_decide} and computationally
verified for all examined residue classes up to $n' = 50000$.  Two sub-families remain
open at the level of formal proof: fixed positions $m \in \{4, 6, \ldots\}$ whose
minimum-witness size grows with a self-similar hierarchy (currently reduced to two
period-8 axioms for $m=4$ via a right-edge co-moving witness construction), and the
right-boundary family $m = 2n'{-}6$ (density~$\tfrac{1}{2}$, mod-4 rule
computationally verified with no exceptions to $n'=50000$).  Closing this lemma
completes an axiom-free Lean~4 proof of $\mathrm{bs}(\mathtt{rule30}_n) \geq n$.

\textbf{What this paper does not prove (Prize~3).}  Wolfram Prize~3 asks whether
computing $s(n) = \mathtt{rule30}_n(e_n)$ from the \emph{fixed} single-spike input
requires $\Omega(n)$ effort on a Turing machine.  Because $e_n$ is a structured input
computable from $n$ alone, an algorithm for the fixed-input problem need not query any
cell --- so our query-complexity result does not directly imply Prize~3.  Two candidate
paths to bridge the gap are discussed in Section~\ref{sec:discussion}: a time-bounded
Kolmogorov complexity argument, and a circuit-complexity argument.  Neither is currently
complete; both require establishing that the center-column sequence $s(n)$ resists
sub-linear extraction in a model that applies to fixed inputs.
...
The period-256 claim therefore does not extend to the full SubcaseB set.
Computational analysis (2026-04-09) shows that SubcaseB continues for all even $m \geq 40$,
with first events occurring at exponentially large $n'$.
...
analysis of $(F(n,m), G(n,m))$ reveals:

\begin{itemize}
\item \emph{Active}: $M_{\mathrm{act}} = \{4, 6, 8, 10, 12, 14, 16, 20, 22,
  24, 26, 28, 30, 34, 36, 38\} \cup \{40, 42, 44, 46, \ldots\}$ (all even $m \geq 4$
  except $m \in \{2, 18, 32\}$).
  For $m \leq 38$: periods range from~8 to~$32768$; first SubcaseB hits lie
  in $[3087, 8210]$; all verified by direct simulation.
  For $m \geq 40$: first SubcaseB events occur at exponentially large $n'$:
  $m=40$ at $n'=40983$ ($P_{40}=65536$),
  $m=42$ at $n'=118804$ ($P_{42}=131072$),
  $m=44$ at $n'=249877$ ($P_{44}=262144$),
  $m=46$ at $n'=106522$ ($P_{46}=524288$).
  All verified by direct Rule~30 simulation.
  Note: Earlier scans over $[3087, 45000)$ reported zero SubcaseB for $m \geq 40$
  because the first events lie far beyond this range ($40983 \gg 45000$ for $m=40$).
\item \emph{Inactive} ($(0,1)$ never occurs): $m \in \{2, 18, 32\}$ only.
  For $m=2$: proved in Lean (\texttt{ts2\_last\_always\_false}).
  For $m=18$: exhaustive $G$-check of all $F{=}0$ candidates in $[3087,3600)$
  yields zero SubcaseB; period-256 structure verified.
  For $m=32$: all 2034 $F{=}0$ candidates in one full period $[3087,7183)$
  exhaustively $G$-checked, zero SubcaseB (loop-20, 245\,s);
  rigorously confirming permanent inactivity via periodicity.
  Completeness of inactive set within $[4,38]$ verified: all even $m \in [4,38]$
  other than $m=18,32$ are confirmed active by direct SubcaseB scan (loop-20).
\end{itemize}

The inactive positions $m=18$ and $m=32$ exhibit $(1,0)$ events but never $(0,1)$:
$m=18$ has $(1,0)$ at $n' \in \{3280, 3336, 3340, \ldots\}$;
$m=32$ has $(1,0)$ at $n' \in \{3347, 4111, 4115\}$.
The interaction term $I(n',m) := F \oplus G \oplus 1$ satisfies $I=0$ only
when $F=1$---SubcaseB requires $F=0$ and $I=0$ simultaneously, which never occurs.

\textbf{LFSR structure of $G_{X,m}$ witness sequences (2026-04-09).}
For each active $m \geq 40$, a parity-clean spike position $X=X(m)$ exists such that
$G_{X,m}(n') \neq F_X(n')$ at the first SubcaseB event.  The witness formula is:
\begin{equation}
  X(m) = \begin{cases} m/2 - 18 & m \equiv 0 \pmod{4} \\ 2 & m \equiv 2 \pmod{4} \end{cases}
\end{equation}
verified for $m=40,42,44,46,48$ by direct simulation.

Lean machine-checked witnesses for the first SubcaseB event at each $m$:
\begin{itemize}
  \item $m=40$: $G_{2,40}(40984)=1 \neq 0 = F_2(40984)$ \hfill(\texttt{x2\_witnesses\_m40})
  \item $m=42$: $G_{2,42}(118805)=0 \neq 1 = F_2(118805)$ \hfill(\texttt{x2\_witnesses\_m42})
  \item $m=46$: $G_{2,46}(106523)=0 \neq 1 = F_2(106523)$ \hfill(\texttt{x2\_witnesses\_m46})
  \item $m=44$: $G_{4,44}(249878) \neq F_4(249878)$ \hfill(\texttt{x4\_witnesses\_m44}, building)
  \item $m=48$: $G_{6,48}(262168) \neq F_6(262168)$ \hfill(\texttt{x6\_witnesses\_m48}, building)
\end{itemize}

The center sequences $G_{X,m}$ have a striking algebraic structure: Berlekamp--Massey
analysis over $\mathrm{GF}(2)$ reveals that the minimal polynomial of each $G_{X,m}$ is
$(1+x)^L$ for some $L$, with $L$ values given in Table~\ref{tab:lfsr}.
\begin{table}[h]
\centering
\begin{tabular}{llrr}
\hline
Sequence & $L$ & $\mathrm{popcount}(L)$ & weight $= 2^{\mathrm{popcount}(L)}$ \\
\hline
$G_{2,40}$ & 57347 & 5 & 32 \\
$G_{2,42}$ & 122884 & 5 & 32 \\
$G_{4,44}$ & 131073 $= 2^{17}+1$ & 2 & 4 \\
$G_{2,46}$ & 262145 $= 2^{18}+1$ & 2 & 4 \\
\hline
\end{tabular}
\caption{LFSR complexity of $G_{X,m}$ witness sequences.  Connection polynomial $(1+x)^L$;
weight $= $ number of nonzero coefficients $= 2^{\mathrm{popcount}(L)}$ exactly.
Verified by BM on 115K--530K terms.}
\label{tab:lfsr}
\end{table}

The $(1+x)^L$ structure implies period divides $2^{\lceil \log_2 L \rceil} = P_m$,
providing period bounds for all $m$ from the algebraic structure alone.
A divisibility chain holds: $C(G_{2,40}) \mid C(G_{2,42}) \mid C(G_{2,44}) \mid C(G_{2,46})$
over $\mathrm{GF}(2)[x]$, verified for all six consecutive pairs.

\begin{conjecture}[$(1{+}x)^L$ structure]
For all even $m \geq 40$ and $X=X(m)$ as above, the minimal polynomial of
$G_{X,m}$ over $\mathrm{GF}(2)$ is $(1+x)^{L(X,m)}$ for an explicit $L$ with
$2^{\lceil \log_2 L \rceil} = P_m = 2^{m/2-2}$.
\end{conjecture}

This conjecture, if proved, provides the algebraic mechanism for closing
\texttt{subcaseB\_mgt38\_witness} for all $m$: the period bound follows from $(1+x)^L$,
and sensitivity transfer extends each finite witness to all periodic repetitions.

\textbf{Proof plan.}  A formal SubcaseB closure has three parts.

\emph{Part A -- inactive positions.}  For $m=2$: already proved.  For $m \in \{18, 32\}$:
prove $F(n,m)=0 \Rightarrow G(n,m)=0$ for all $n \geq 3087$;
computationally established for $m=32$ (exhaustive scan); $m=18$ pending formal period-certificate proof.
...
\begin{itemize}
  \item \emph{Inactive} ($m \in \{18, 32\}$): show $F=0 \Rightarrow I=1$ for all $n' \geq 3087$.
    $m=18$ has period~$256$; $(1,0)$ triplets at offsets~$\{193, 249, 253\}$ per period;
    zero $(0,1)$ in two full periods.
    $m=32$: all 2034 $F{=}0$ candidates in one full period $[3087,7183)$
    exhaustively $G$-checked (loop-20, 245\,s, zero SubcaseB);
    \emph{inactivity for $m=32$ is rigorously established computationally}.
  \item \emph{Active, $m \geq 40$}: the interaction term $I$ does reach~$0$ with $F=0$,
    so SubcaseB fires.  The proof target is to find a parity-clean witness $X=X(m)$ and
    prove $G_{X,m}(n') \neq F_X(n')$ at the first event (and all periodic repetitions).
    The $X(m)$ formula (Eq.~above) provides the witness; Lean \texttt{native\_decide}
    checks the first event for each $m$; \texttt{sensitivity\_transfer} extends to
    all periodic repetitions given period certificates via \texttt{caEvolveU64}.
\end{itemize}

\emph{Part B -- fixed-$m$ active positions ($m \leq 38$).}  For $M_{\mathrm{act}} \cap [4,38]$
(16 positions), the cluster period $P_m$ grows with $m$ (empirical):
\[
...
$P_{40}=65536$ and $P_{42}=131072$ independently confirmed.
\emph{Caution}: earlier resonance-window SubcaseB checks for $m=40$ and $m=42$ over small
$n'$-ranges were incorrectly interpreted as evidence of inactivity; the true first SubcaseB
events occur at $n'=40983$ ($m=40$) and $n'=118804$ ($m=42$), well beyond those scan ranges.)
...
\emph{Run-D: $m=40$ (adversarial loop 1774432000, 2026-03-25)}: with $P_{40}=65536$
confirmed, BM with $N=140000$ samples ($> 2L$) gives:
\[
  L_{40} = 57347,\quad d_{40} = 65536-57347 = 8189 = d_{38}-1.
\]
\textbf{The step-$(-1)$ progression continues through $m=40$}, exactly as the period-doubling
...
\emph{Run-E: $m=42$ (adversarial loop 1774435000, 2026-03-25)}: BM with $N=250000$
...
an exact arithmetic progression with step~$-1$.
\emph{Run-F: $m=44$ (adversarial loop 1774444000, 2026-03-25)}: BM with $N=515000$
($> 2\times 253957 = 507914$) \textbf{confirms} $d_{44}=8187$, $L_{44}=253957$
(this is the $F_2$-based BM; the witness sequence $G_{4,44}$ with $X=4$ gives $L=131073=2^{17}+1$).
...
In contrast, $m\geq 40$ (\textbf{correction}: now known to be active, not ``block inactive'')
follow both laws: $P_{40}=2P_{38}=65536$ and $d_{40}=d_{38}-1=8189$ (loop 1774432000, 2026-03-25).
\emph{The doubling law continues for all $m\geq 40$}:
\begin{itemize}
...
\textbf{Correction (2026-04-09)}: Earlier layered sweeps were interpreted as confirming
$m \geq 40$ inactive; this conclusion was \textbf{wrong}.
The first SubcaseB event for $m=40$ occurs at $n'=40983$, far beyond the scanned range $[3087,7000)$
(and even beyond the resonance test at $n'=36887$).
All prior scans for $m \geq 40$ covered insufficient $n'$-range to detect the first SubcaseB events.
The sweep data below is retained as a historical record; its inactivity conclusion is superseded
by direct simulation of the first events (Section above).

The sweeps covered $n'$-ranges that were insufficient:
\begin{itemize}
\item $m=40$: scans over $[3087,7000)$ and resonance tests at $n'=16403, 36887$ all gave
  0 SubcaseB — correctly, since first event is at $n'=40983$.
  The $(1,0)$ at $n'=36887$ is a genuine non-SubcaseB interaction; but SubcaseB does occur
  later at $n'=40983$ ($(F,G)=(0,1)$ confirmed by direct simulation, 2026-04-09).
...
\end{itemize}
The inactive set is $\{2, 18, 32\}$ only.  The active set is all even $m \geq 4$ except these:
\[
  M_{\mathrm{act}} = \{4, 6, 8, 10, 12, 14, 16, 20, 22, 24, 26, 28, 30, 34, 36, 38, 40, 42, 44, 46, \ldots\}.
\]
...
in $[4,38]$ correctly (though the prediction $m=50$ inactive is \textbf{unverified} and
likely wrong; see correction below).
\emph{Correction (2026-04-09)}: the criterion fails beyond $[4,38]$: $m=40$ and $m=42$
are \emph{active} (SubcaseB fires at $n'=40983$ and $118804$ respectively), contradicting
the criterion's prediction.  The 2-adic coincidence within $[4,38]$ has no known explanation
and does not extend to larger $m$.
...
\end{itemize}
In contrast, $m\geq 40$ (now known to be active) have $L_m = P_m - (8191-(m-36)/2)$
(the arithmetic progression, no Mersenne structure), and active $m \leq 38$ have $L_m$ values
with no Mersenne factor.  Whether the Mersenne-inner property is a cause or consequence of
sporadicity is an open question.
...
\textbf{Correction (2026-04-09):} The active set is infinite.
Direct simulation finds SubcaseB events for all even $m \geq 40$, with first occurrences
at exponentially large $n'$ (see Section above).  The overall period $P$ grows without bound
as $m \to \infty$; the fixed-$m$ family has infinite scope and the period is not finite.
The axiom \texttt{subcaseB\_mgt38\_witness} (open) asserts existence of parity-clean witnesses
for all $m \geq 40$; verified for $m \in \{40,42,44,46,48\}$.
\begin{enumerate}
...
\textbf{Correction (2026-04-09)}: The assumption of no active $m > 38$ is \textbf{false}.
All even $m \geq 40$ are active (SubcaseB fires at exponentially large $n'$);
all prior scans over $[3087, 20001)$ were far below the first events.
The overall period $P = \mathrm{lcm}(P_m : m \in M_{\mathrm{act}})$ grows without bound.
Part~B has infinite scope; closing it requires an algebraic argument for all $m$.
\end{sloppypar}

The open subproblem is to close \texttt{subcaseB\_mgt38\_witness} for \emph{all} even $m\geq 40$
algebraically (the witness formula $X(m)$ is conjectured but unproved in general;
and odd $m$ remain unchecked for large $n'$).
...
\end{itemize}
\textbf{Correction (2026-04-09)}: these resonance checks targeted the wrong $n'$-range.
$m=46$ first SubcaseB occurs at $n'=106522$; $m=48$ at $n'=262167$ (direct simulation).
The six resonance checks above were all below the true first events.
$m=46,48,50$ are \emph{active}, not inactive.
...
\emph{$m=40$ F-sequence (loop 1774432000, 2026-03-25)}: $d_{40}=8189$ (spacing$=1$, full poly);
...
\textbf{Correction (2026-04-09)}: $m=40$ is \emph{active} (first SubcaseB at $n'=40983$);
the earlier ``block inactive'' label was incorrect.
The sporadic inactive~$m\in\{18,32\}$ within active blocks have small defects ($d=1$ or $4$)
reflecting their isolated position; $m\geq 40$ follows the same arithmetic progression as active~$m$,
because $m\geq 40$ IS active.
A valid GF(2) fingerprint for the \emph{sporadic} inactive set $\{2,18,32\}$ exists at the
\emph{indicator polynomial} level:
$\sum_{m\in\{2,18,32\}} y^{m/2} = 1+y+y^9+y^{16}$
...
active~$m$ have $I(n',m)=0$ at periodic $n'$ (SubcaseB); inactive~$m\in\{2,18,32\}$ have
$I(n',m)\equiv 1$ for all~$n'\geq 3087$.
...
whether the $(1+x)^L$ conjecture for $G_{X,m}$ sequences can be proved in general
(if so, period bounds and witness certs follow for all $m$, closing \texttt{subcaseB\_mgt38\_witness}).
...
\subsection*{Bridge to Prize~3: two proof paths}

Prize~3 asks whether $s(n) = \mathtt{rule30}_n(e_n)$, computed from index $n$ alone,
requires $\Omega(n)$ time on a Turing machine.  Our Theorem~1.3 gives $\Omega(n)$
query complexity for \emph{general} inputs $c$; the gap is that an algorithm for the
fixed-input problem need not ``query'' any cell of $e_n$, since every cell of $e_n$ is
immediately computable from $n$.  Two paths could close this gap:

\textbf{Path~1: Kolmogorov incompressibility.}  Prize~3 can be restated as: the
time-bounded Kolmogorov complexity $KT_{o(n)}(s(0)\cdots s(n) \mid n) = \Omega(n)$,
meaning no program of length $o(n)$ \emph{running in $o(n)$ steps} computes the first
$n$ center-column values.  This is strictly stronger than the unconditional statement
$K(s(0)\cdots s(n) \mid n) = \Omega(n)$, which bounds only description length, not
running time---a long-description program could in principle run quickly, and a
short-description program could run for exponential time.  Proving either form requires
showing the center-column sequence resists extraction without simulation: that it carries
$\Omega(1)$ bits of new information per step that cannot be decoded in sublinear time.
Wolfram's empirical investigations and the density-$\frac{1}{2}$ result (computationally
confirmed; Prize~2 open formally) support this conjecture, but no proof exists.

\textbf{Path~2: Circuit complexity.}  If every Boolean circuit family computing
$n \mapsto s(n)$ has size $\Omega(n)$, an $o(n)$-step TM would violate this by
producing a sublinear-size implicit circuit, giving the lower bound.  Our
block-sensitivity result ($\mathrm{bs}(\mathtt{rule30}_n) \geq n$) applies to the
general-input function; connecting this to the circuit complexity of the single-valued
sequence $n \mapsto s(n)$ requires a different argument.

\textbf{Connection between the paths.}  Prize~1 (non-periodicity) and Prize~3 are
related: if $s(n)$ were periodic with period $p$, a $O(\log n)$-time algorithm would
exist.  Prize~1 being open means even the weakest form of Prize~3 is unresolved.
Conversely, a proof of Prize~3 would imply Prize~1 as an immediate corollary.  The
Kolmogorov path therefore also constitutes a strengthening of Prize~1's non-periodicity
claim.

\textbf{What the present work contributes toward the bridge.}  The all-cells-essential
result establishes that Rule~30's dynamics are genuinely \emph{non-degenerate}: no
initial cell is redundant, no causal pathway is short-circuited.  This is the
structural fact that makes computational irreducibility plausible.  A formal connection
would require either (a) an algebraic argument showing the center-column bits cannot
satisfy a linear recurrence of degree $o(n)$ over $\mathrm{GF}(2)$ (our LFSR analysis
shows the active-$m$ sequences have connection polynomials of degree $\Omega(m)$,
suggesting the center column itself has high LFSR complexity), or (b) a direct appeal
to the nonlinearity of Rule~30's OR gate, which prevents the kind of algebraic shortcut
available in Rule~90.
