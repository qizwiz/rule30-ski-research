...
\begin{abstract}
\textbf{What this paper proves.}  We prove that every position in the width-$n$ light
cone of Rule~30 is \emph{essential} for \emph{general} inputs: for each
position~$k \in \{0,\ldots,2n\}$, there exists an initial configuration $c$ such that
the $n$-step center-cell output changes when $c[k]$ is flipped.  Since every input cell
is essential, any algorithm computing $\mathtt{rule30}_n(c)$ for an arbitrary
$c$ must read all $2n+1$ input cells in the worst case, yielding an $\Omega(n)$
\emph{query-complexity} lower bound for the \emph{general-input} problem.

\textbf{What this paper does not prove.}  Wolfram Prize~3 asks a different question:
does computing $s(n) = \mathtt{rule30}_n(e_n)$, the $n$-th center-column value from
the \emph{fixed} single-spike input $e_n$, require $\Omega(n)$ effort on a Turing
machine?  Because $e_n$ is always the same structured input and every cell of $e_n$ is
immediately computable from $n$, an algorithm for the fixed-input problem need not
``query'' any cell of $e_n$ at all.  Our query-complexity result therefore does not
directly imply Prize~3; an additional bridge argument is needed, and two candidate
paths are discussed in Section~\ref{sec:discussion}.
...
\textbf{Path~1: Kolmogorov incompressibility.}  If the conditional Kolmogorov
complexity $K(s(0)\cdots s(n) \mid n) = \Omega(n)$, then no program of length $o(n)$
computes the first $n$ center-column values given $n$, and in particular no $o(n)$-step
TM does.  This path requires proving that the center-column sequence resists
compression---essentially that it carries $\Omega(1)$ bits of new information per step.
Wolfram's empirical investigations and the density-$\frac{1}{2}$ result (computationally
confirmed; Prize~2 open formally) support this, but no proof exists.
