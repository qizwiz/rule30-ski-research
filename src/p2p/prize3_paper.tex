...
\begin{remark}[Arithmetic structure of inactive positions within $[4,38]$]
\label{rem:inactivestructure}
The two inactive positions in $[4,38]$ share a 2-adic structure:
$m=18$ has $v_2(18)=1$ with odd part $9=3^2$ (a perfect square);
$m=32$ has $v_2(32)=5$ (a pure high power of~2).
The 16 active positions satisfy neither condition.
This pattern correctly classifies all 18 even positions in $[4,38]$ and
predicts $m=50$ inactive ($\mathrm{odd}(50)=25=5^2$), consistent with scans.
\emph{However}, the criterion fails immediately beyond $[4,38]$:
$m=40$ ($v_2=3$, $\mathrm{odd}=5$) and $m=42$ ($v_2=1$, $\mathrm{odd}=21$, not a square)
are both confirmed inactive despite satisfying neither condition.
Inactivity appears to be a dynamical property of the Rule~30 spike-propagation,
not an arithmetic one; the 2-adic coincidence within $[4,38]$ has no known explanation.
\end{remark}
